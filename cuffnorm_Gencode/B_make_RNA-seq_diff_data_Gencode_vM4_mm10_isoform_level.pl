...
#REF#################################
open(REF, "</home/akimitsu/custom_command/cuffnorm_Gencode/mRNA_lncRNA_list_gencode_vM4_mm10_isoform_level.txt") || die;
...
#REF#################################
open(REF, "<$input/isoforms.attr_table") || die;
...
#MAIN################################
open(IN,"<$input/isoforms.fpkm_table") || die;
open(OUT_mRNA,">$input/isoforms_Gencode_vM4_result_mRNA.fpkm_table") || die;
open(OUT_ncRNA,">$input/isoforms_Gencode_vM4_result_ncRNA.fpkm_table") || die;
...
while(my $line = <IN>){
	...
	#print "$type\n";
	if($type eq "mRNA"){
		print OUT_mRNA "$gr_id_mRNA\t$gene_name\t$gene_id\t$locus\t",join("\t",@result),"\n";
		$gr_id_mRNA++;
	}elsif($type eq "ncRNA"){
		print OUT_ncRNA "$gr_id_ncRNA\t$gene_name\t$gene_id\t$locus\t",join("\t",@result),"\n";
		$gr_id_ncRNA++;
	}
}
