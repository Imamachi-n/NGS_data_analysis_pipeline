...
################################
open(IN,"</home/akimitsu/gencode.v19.annotation.gtf") || die;
open(OUT,">./mRNA_lncRNA_list_gencode_v19_hg19_isoform_level.txt") || die;
...
foreach(@ref_gene_list){
	...
	if($type eq "protein_coding"){
		foreach(@trx_id){
			print OUT "$gene_id\tmRNA\t$type\t$_\t$name\n";
		}
	}else{
		foreach(@trx_id){
			print OUT "$gene_id\tncRNA\t$type\t$_\t$name\n";
		}
	}
}
